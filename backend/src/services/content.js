...
/**
 * Which items this device has already completed — for UI restore after
 * reload. Includes `earn_secs` so the frontend can rebuild an accurate
 * unclaimed balance (sum of rows where claimed = false) instead of losing
 * track of it on a fresh page load.
 */
export async function getClientCompletions(macAddress) {
  const { rows } = await query(
    `SELECT cc.content_item_id, cc.claimed, cc.earn_secs
 ...
 * was already completed earlier (no double-crediting), matching the
 * `UNIQUE (client_id, content_item_id)` constraint on content_completions.
 */
export async function recordCompletion(macAddress, contentItemId, { elapsedSecs = 0, response = null } = {}) {
  ...
  const { rows } = await query(
    `INSERT INTO content_completions (client_id, content_item_id, earn_secs, response)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (client_id, content_item_id) DO NOTHING
     RETURNING earn_secs`,
    [clientId, contentItemId, item.earn_secs, response ? JSON.stringify(response) : null]
  );
...
export async function createContentItem({
  ...
  const { rows } = await query(
    `INSERT INTO content_items
       (type, section, title, category, duration_label, earn_secs, min_watch_secs, img_url, body_url, survey_questions, sort_order)
